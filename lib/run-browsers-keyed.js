...
function runBrowsersKeyed(location, remote, platforms, options) {
  ...
  var results = {passedBrowsers: [], failedBrowsers: []};
  return Promise.all(Object.keys(platforms).map(function (key) {
    ...
    return run(location, remote, platforms[key], {
      ...
      results[key] = result;
      results.passedBrowsers = results.passedBrowsers
                                      .concat(result.passedBrowsers);
      results.failedBrowsers = results.failedBrowsers
                                      .concat(result.failedBrowsers);
